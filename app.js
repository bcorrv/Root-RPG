(() => {
  ...
  function moveToMission(m){
    ...
    chooseClear(`Mover hacia · ${m.title}`,c=>{
      if(String(c)===String(S.character.clear))return toast('Ya estás en ese claro');
      if(recordAction({actionId:'move',label:`Mover hacia · ${m.title}`,icon:'move',cost:1,clear:c,meta:`Objetivo: claro ${m.targetClear}`})){
        if(String(c)===String(m.targetClear))m.stage='scene';
