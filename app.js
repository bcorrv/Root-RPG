(() => {
  const KEY='rootRpg_v06';
  const LEGACY=['rootRpg_v04','rootRpg_v03','rootRpg_v02','rootSandboxCompanion_v02','rootSandbox_v01'];
  ...
  function migrate(old){
    ...
    n.states=Array.isArray(old.states)?old.states:[];n.rumors=Array.isArray(old.rumors)?old.rumors:[];n.missions=Array.isArray(old.missions)?old.missions:[];
    n.landmarks=Array.isArray(old.landmarks)?LANDMARKS.map(base=>{const f=old.landmarks.find(x=>x.id===base.id||String(x.name||'').includes(base.name.split(' ')[0]));return f?{...base,level:f.level||0,clear:f.clear||''}:{...base,level:0,clear:''}}):n.landmarks;
    n.log=Array.isArray(old.log)?old.log:[];n.actions=Array.isArray(old.actions)?old.actions:[];
    ...
    if(oc.vagabondType&&VAGS[oc.vagabondType]){
      ...
      n.character.skills=oc.skills||[];
      n.character.milestones=oc.milestones||[];
      n.character.pendingSkillChoices=oc.pendingSkillChoices||0;
      n.character.discoveries=oc.discoveries||[];
      n.character.importantEvents=oc.importantEvents||[];
      n.character.clear=oc.clear||'';
      n.onboarded=true;
    }else{
  ...
  function signatureActions(){return{thief:['infiltrate','trade','investigate'],ranger:['explore','combat','move'],tinker:['craft','explore','investigate']}[S.character.vagabondType]||[]}
  ...
  function finishWorld(){archiveOrders();addLog('Fase del Mundo cerrada.','world');S.day++;S.time=0;S.phase='player';S.character.currentItems.filter(i=>i.status==='exhausted').forEach(i=>i.status='ready');S.world={day:S.day,resolved:{Marquesado:false,Eyrie:false,Alianza:false},records:{}};save();closeSheet();renderAll();switchView('turno');toast(`Día ${S.day}`)}

  function resolveMission(m,optionLabel='Resolver escena'){diceResolver({title:optionLabel,label:`Misión · ${m.title}`,icon:'star',actionId:'investigate',cost:1,meta:optionLabel,after:band=>{if(band.success){m.stage='reward';m.routeUsed=optionLabel}else addLog(`Complicación en ${m.title}`,'alert')}})}
  function progressLead(m){if(recordAction({actionId:'investigate',label:`Investigar pista · ${m.title}`,icon:'search',cost:1})){chooseClear('Localizar objetivo',c=>{m.targetClear=String(c);m.stage='travel';save();closeSheet();renderMissions()})}}
  function enterScene(m){m.stage='scene';save();renderMissions()}
  function claimReward(m){S.character.currentItems.push(item(m.reward,'ready','mission'));m.status='completed';m.stage='done';addLog(`Misión cumplida: ${m.title} · obtienes ${ITEM_DEF[m.reward].name}`,'star');evaluateMilestones();save();renderAll();toast('Recompensa obtenida')}
  ...
  function renderCharacter(){
    const v=VAGS[S.character.vagabondType];if(!v)return;
    $('#characterPortrait').src=v.portrait;$('#characterName').textContent=S.character.name;$('#characterArchetype').textContent=v.name;$('#characterTypeLabel').textContent=v.playstyle;$('#characterTagline').textContent=v.tagline;
    $('#specialAbilityCard').innerHTML=`<div class="ability-kicker">Habilidad única</div><div class="ability-title">${v.specialAbility.name}</div><div class="ability-copy">${v.specialAbility.description}</div><button class="chip" id="useAbilityFromSheet">Usar habilidad</button>`;$('#useAbilityFromSheet').onclick=useSpecialAbility;
    const ready=$('#skillReadyBanner');ready.innerHTML=S.character.pendingSkillChoices>0?`<div class="skill-ready"><div><strong>Nueva habilidad disponible</strong><span>Tienes ${S.character.pendingSkillChoices} elección pendiente.</span></div><button class="btn gold" id="jumpSkills">Elegir</button></div>`:'';const js=$('#jumpSkills');if(js)js.onclick=()=>document.getElementById('skillTree').scrollIntoView({behavior:'smooth'});
  }
  ...
  function renderRumors(){const b=$('#rumorsList');$('#rumorCounter').textContent=`${S.rumors.length} / 3`;b.innerHTML=S.rumors.length?'':'<div class="empty">Sin rumores activos.</div>';S.rumors.forEach((r,i)=>{const f=FACTIONS[r.origin]||FACTIONS.Marquesado,d=document.createElement('div');d.className='rumor-card';d.style.setProperty('--rumor-color',f.color);d.innerHTML=`<div class="rumor-head"><div class="rumor-source"><div class="faction-seal ${f.className}">${ico(f.icon)}</div><div><div class="item-title">${r.origin}</div><div class="item-meta">${SUITS[r.suit]?.name} · Día ${r.day}</div></div></div></div><div class="rumor-copy">“${esc(r.text)}”</div><div class="btn-row" style="margin-top:10px"><button class="btn" data-f>Seguir pista</button><button class="btn secondary" data-r>Dejar morir</button></div>`;d.querySelector('[data-f]').onclick=()=>{const m=r.seed||missionDraft(r.origin,r.suit,['Movió']);m.stage='lead';S.missions.push(m);S.rumors.splice(i,1);save();renderAll()};d.querySelector('[data-r]').onclick=()=>{S.rumors.splice(i,1);save();renderRumors()};b.appendChild(d)})}
  function renderMissions(){const b=$('#missionsList'),ms=S.missions.filter(m=>m.status==='active');$('#missionCounter').textContent=`${ms.length} activas`;b.innerHTML=ms.length?'':'<div class="empty">Las Órdenes pueden abrir misiones.</div>';ms.forEach(m=>{const f=FACTIONS[m.origin],reward=ITEM_DEF[m.reward],d=document.createElement('div');d.className='mission';d.style.setProperty('--mission-color',f.color);let action='';
      if(m.stage==='lead')action=`<button class="btn" data-lead>Investigar pista</button>`;
      if(m.stage==='travel')action=S.character.clear===m.targetClear?`<button class="btn" data-enter>Entrar a la escena</button>`:`<button class="btn secondary" data-journey>Objetivo · claro ${m.targetClear}</button>`;
      if(m.stage==='scene'){const opts=missionOptions(m);action=`${opts.map((o,i)=>`<button class="btn gold" data-skill="${i}">${esc(o.label)} · ${esc(o.skill)}</button>`).join('')}<button class="btn" data-generic>Resolver escena</button>`}
      if(m.stage==='reward')action=`<button class="btn gold" data-reward>Tomar ${reward.name}</button>`;
      d.innerHTML=`<div class="mission-ribbon"></div><div class="mission-inner"><div class="mission-head"><div><div class="mission-kicker">${m.origin} · ${SUITS[m.suit]?.name||''}</div><div class="mission-title">${esc(m.title)}</div></div><div class="faction-seal ${f.className}">${ico(f.icon)}</div></div><div class="mission-objective">${esc(m.objective)}</div><div class="mission-stage">Etapa · ${{lead:'Pista',travel:'Localizada',scene:'Escena',reward:'Recompensa'}[m.stage]}</div><div class="mission-actions dynamic-actions">${action}</div></div>`;
      const q=s=>d.querySelector(s);if(q('[data-lead]'))q('[data-lead]').onclick=()=>progressLead(m);if(q('[data-enter]'))q('[data-enter]').onclick=()=>enterScene(m);if(q('[data-journey]'))q('[data-journey]').onclick=()=>switchView('turno');if(q('[data-generic]'))q('[data-generic]').onclick=()=>resolveMission(m);d.querySelectorAll('[data-skill]').forEach(x=>x.onclick=()=>resolveMission(m,missionOptions(m)[+x.dataset.skill].label));if(q('[data-reward]'))q('[data-reward]').onclick=()=>claimReward(m);b.appendChild(d)})}
